...
public record ItemStackVarItem(ItemStack item, SpanData spanData) implements Value {
    ...
    public void lowerToParsable(StringBuilder builder, int depth) {
        ...
        if(item.getCount() != 1 || !patch.isEmpty()) {
            builder.append(", ")
                    .append(item.getCount());
        }

        if(!patch.isEmpty()) {
            var encoded = DataComponentPatch.CODEC.encodeStart(RegistryOps.create(NbtOps.INSTANCE, Minecraft.getInstance().player.registryAccess()), patch).getOrThrow();
            var literal = new NbtLiteral(encoded, spanData);
            builder.append(", ");
            literal.lowerToParsable(builder, depth);
        }
        builder.append(")");
    }
